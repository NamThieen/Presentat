...
class PresentatWindow(Adw.ApplicationWindow):
    ...
    def __init__(self, **kwargs):
        ...
        self.tree_list_model = Gtk.TreeListModel.new(
            self.root_list_store,
            False,
            False,
            create_child_model_func
        )
    ...
    def _factory_bind(self, factory, list_item: Gtk.ListItem):
        ...
        try:
            file_info = item.file.query_info("standard::icon", Gio.FileQueryInfoFlags.NONE, None)
    ...
    def on_save_response(self, dialog, result):
        try:
            ...
            self.toast_overlay.add_toast(Adw.Toast(title=f"Error saving file: {e.message}"))
    ...
    def _on_file_saved(self, file, result):
        ...
        if success:
            ...
            self.toast_overlay.add_toast(Adw.Toast(title=f"Saved as {display_name}"))
        else:
            self.toast_overlay.add_toast(Adw.Toast(title=f"Error saving file: {message}"))
    ...
    def on_open_response(self, dialog, result):
        try:
            ...
            self.toast_overlay.add_toast(Adw.Toast(title=f"Error opening file: {e.message}"))

    def open_file(self, file):
        if self.file_manager.get_file_type(file) == Gio.FileType.DIRECTORY:
            self.toast_overlay.add_toast(Adw.Toast(title="Cannot open directory as a file."))
            return
    ...
    def _on_file_loaded(self, file, result):
        ...
        if success:
            ...
            self.toast_overlay.add_toast(Adw.Toast(title=f"Unable to open file: {content}"))
    ...
    def on_open_folder_response(self, dialog, result):
        try:
            ...
            if folder is not None:
                ...
                self.toast_overlay.add_toast(Adw.Toast(title=f"Opened folder: {folder.get_basename()}"))
            else:
                self.toast_overlay.add_toast(Adw.Toast(title="No folder selected."))
        except GLib.Error as e:
            self.toast_overlay.add_toast(Adw.Toast(title=f"Error opening folder: {e.message}"))
